...
export function ProductCatalog({ initialProducts }: { initialProducts: any[] }) {
    ...
    const filteredProducts = useMemo(() => {
        return enhancedProducts.filter((product) => {
            ...
            return (
                matchesSearch && matchesCategory && matchesBuildType && matchesCapacity && isActive
            );
        });
    }, [searchQuery, selectedCategories, selectedBuildTypes, selectedCapacities, enhancedProducts]);
                ...
                <div className="container mx-auto px-4 md:px-6">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <Sheet open={mobileFilterOpen} onOpenChange={setMobileFilterOpen}>
                                <SheetTrigger asChild>
                                    <button className="lg:hidden flex items-center gap-2 px-4 py-2.5 bg-muted rounded-lg text-sm font-medium text-foreground hover:bg-muted/80 transition-colors">
                    ...
                    {activeFilterCount > 0 && (
                        <div className="flex flex-wrap items-center gap-2 mb-6">
                            {[...selectedCategories, ...selectedBuildTypes, ...selectedCapacities].map((filter) => (
                                <button
                                    key={filter}
                                    onClick={() => {
                                        setSelectedCategories((prev) => prev.filter((f) => f !== filter));
                                        setSelectedBuildTypes((prev) => prev.filter((f) => f !== filter));
                                        setSelectedCapacities((prev) => prev.filter((f) => f !== filter));
                                    }}
                                    ...
                                    <motion.div key={product.id} variants={itemVariants}>
                                        <Card className="group overflow-hidden border border-border hover:border-primary/30 hover:shadow-lg transition-all duration-500 bg-white h-full flex flex-col">
                                            <div className="relative h-56 overflow-hidden bg-muted flex-shrink-0">
                                                ...
                                                {(product.buildType === "Heavy Duty" || product.status === "Active") && (
                                                    <Badge
                                                    ...
                                                    >
                                                        {product.buildType === "Heavy Duty" ? "Heavy Duty" : "Best Seller"}
                                                    </Badge>
                                                )}
                                            </div>
